...
'''' Author                     Created Date
'''' Suresh                      20/01/2009
'''' <remarks> This class is used for entering the Parameters for the Machine Utilization Report.</remarks>
Public Class MCUtilReport
    Inherits GeneralLib
    ''' <summary>
...
#Region "Variable Declaration"
    ...
    Private WithEvents MCUtilChildReportClass As MCUtilChildReport
#End Region
    ''' <summary>
    ''' SetApplication() method is called to connect the application through the connection string.
    ''' LoadFromXML("FrmMCUtilReport.srf") method is called to load the Production Report form.
    ...
    Public Sub New(ByVal aSBO_Application As SAPbouiCOM.Application, ByVal aCompany As SAPbobsCOM.Company)
        ...
        oCompany = aCompany
        LoadFromXML("FrmMCUtilReport.srf")
        DrawForm()
    End Sub
    ''' <summary>
...
& "Group by T0.U_wcno"
                                ...
                                If oRs1.RecordCount > 0 Then
                                    ...
                                    For i = 0 To oRs1.RecordCount - 1
                                        ...
                                        'Modified by Manimaran-----------s
                                        '                                    StrSql = "Select Tbl3.Machine,Tbl3.Avl,Tbl3.Wrk,Tbl3.Stp, " _
                                        '& "Case When Tbl3.Util <= 100 then Tbl3.Util Else '100' End from  " _
                                        '& "(select T0.U_wcno as 'Machine',Tbl2.Avl as 'Avl',Tbl.Wrk as 'Wrk',Tbl1.Stp as 'Stp', " _
                                        '& "Convert(float,(convert(float,Tbl.Wrk) / convert(float,Tbl2.Avl)) * 100)  as 'Util' " _
                                        '& "from [@PSSIT_PMWCHDR] T0 inner join  [@PSSIT_PMWCSFT] T1 on T1.Code=T0.Code  " _
                                        '& "inner join (select T0.U_wcno,Convert(float,(Sum(Convert(float,T1.U_Duratmin))/60)) " _
                                        '& "as 'Avl' from [@PSSIT_PMWCSFT] T1 " _
                                        '& "inner Join  [@PSSIT_PMWCHDR] T0 on T0.code=T1.Code where T0.U_wcno='" & oMachNo & "' " _
                                        '& "Group by T0.U_wcno) Tbl2 on Tbl2.U_wcno=T0.U_wcno " _
                                        '& "Inner Join (Select T2.U_wcno ,Convert(float,(Sum(Convert(float,T2.U_rntime))/60))  " _
                                        '& "as 'Wrk' from [@PSSIT_PEY1] T2 " _
                                        '& "inner join [@PSSIT_PMWCHDR] T0  on T0.U_wcno=T2.U_wcno where T0.U_wcno='" & oMachNo & "' " _
                                        '& "group by T2.U_wcno) Tbl On Tbl.U_wcno = T0.U_wcno " _
                                        '& "Inner Join (select T4.U_deptcode ,Convert(Float,(Sum(Convert(float,T4.U_totmts))/60)) as 'Stp' " _
                                        '& "from [@PSSIT_PMWCBREAKHDR] T4 inner join [@PSSIT_PMWCHDR] T0  " _
                                        '& "on T0.U_wcno =T4.U_deptcode where T0.U_wcno='" & oMachNo & "' " _
                                        '& "group by T4.U_deptcode) Tbl1 on Tbl1.U_deptcode =T0.U_wcno " _
                                        '& "where T0.U_wcno='" & oMachNo & "' and T1.U_Duratmin is not null " _
                                        '& "group by T0.U_wcno,Tbl2.Avl ,Tbl.Wrk,Tbl1.Stp) Tbl3"
                                        StrSql = "Select Tbl3.Machine,Tbl3.Avl,Tbl3.Wrk,Tbl3.Stp, " _
& "Case When Tbl3.Util <= 100 then Tbl3.Util Else '100' End from  " _
& "(select T0.U_wcno as 'Machine',Tbl2.Avl as 'Avl',Tbl.Wrk as 'Wrk',Tbl1.Stp as 'Stp', " _
& "Convert(float,(convert(float,Tbl.Wrk) / convert(float,Tbl2.Avl)) * 100)  as 'Util' " _
& "from [@PSSIT_PMWCHDR] T0 inner join  [@PSSIT_PMWCSFT] T1 on T1.Code=T0.Code  " _
& "inner join (select T0.U_wcno,Convert(float,(Sum(Convert(float,T1.U_Duratmin)))) " _
& "as 'Avl' from [@PSSIT_PMWCSFT] T1 " _
& "inner Join  [@PSSIT_PMWCHDR] T0 on T0.code=T1.Code where T0.U_wcno='" & oMachNo & "' " _
& "Group by T0.U_wcno) Tbl2 on Tbl2.U_wcno=T0.U_wcno " _
& "Inner Join (Select T2.U_wcno ,Convert(float,(Sum(Convert(float,T2.U_rntime))))  " _
& "as 'Wrk' from [@PSSIT_PEY1] T2 " _
& "inner join [@PSSIT_PMWCHDR] T0  on T0.U_wcno=T2.U_wcno where T0.U_wcno='" & oMachNo & "' " _
& "group by T2.U_wcno) Tbl On Tbl.U_wcno = T0.U_wcno " _
& "left outer Join (select T4.U_deptcode ,Convert(Float,(Sum(Convert(float,m.U_StpgTime)))) as 'Stp' " _
& "from [@PSSIT_PMWCBREAKHDR] T4 inner join [@PSSIT_PMWCREASONDTL] m on m.DocEntry = t4.DocEntry  inner join [@PSSIT_PMWCHDR] T0  " _
& "on T0.U_wcno =T4.U_deptcode where T0.U_wcno='" & oMachNo & "' " _
& "group by T4.U_deptcode) Tbl1 on Tbl1.U_deptcode =T0.U_wcno " _
& "where T0.U_wcno='" & oMachNo & "' and T1.U_Duratmin is not null " _
& "group by T0.U_wcno,Tbl2.Avl ,Tbl.Wrk,Tbl1.Stp) Tbl3"
                                        'Modified by Manimaran-------------e
                                        MCUtilChildReportClass = New MCUtilChildReport(SBO_Application, oCompany, StrSql)
                                        oRs1.MoveNext()
