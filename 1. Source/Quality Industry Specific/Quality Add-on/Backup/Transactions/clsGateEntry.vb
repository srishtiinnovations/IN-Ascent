...
Public Class clsGateEntry
    ...
    Public Const formtype As String = "Frm_GE"
    Public Sub LoadScreen()
        Try
            oForm = objAddOn.objUIXml.LoadScreenXML("GateEntry.xml", SST.enuResourceType.Embeded, formtype)
            oForm.Items.Item("6").Specific.value = objAddOn.objGenFunc.GetDocNum("SST_GAT")
                    ...
                    Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                        If pVal.ItemUID = "1000001" Then
                            ...
                            If oMatrix.RowCount = 0 Then
                                ...
                                oForm.DataSources.DBDataSources.Item("@SST_GAT1").SetValue("LineId", 0, oMatrix.RowCount)
                                oMatrix.SetLineData(oMatrix.RowCount)
                            Else
                                oForm.DataSources.DBDataSources.Item("@SST_GAT1").Clear()
                                oMatrix.AddRow()
                                oMatrix.GetLineData(oMatrix.RowCount)
                                oForm.DataSources.DBDataSources.Item("@SST_GAT1").SetValue("LineId", 0, oMatrix.RowCount)
                                oMatrix.SetLineData(oMatrix.RowCount)
    ...
    Private Function Validate(ByVal pVal) As Boolean
        ...
        objRS = objAddOn.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        strSQL = "select * from opor where docnum = " & Trim(oForm.DataSources.DBDataSources.Item("@SST_OGAT").GetValue("U_PONum", 0)) & " and cardcode = '" & Trim(oForm.DataSources.DBDataSources.Item("@SST_OGAT").GetValue("U_VCode", 0)) & "'"
        objRS.DoQuery(strSQL)
        If objRS.RecordCount = 0 Then
    ...
    Private Sub AddPOCon()
        ...
        oCon.Alias = "CardCode"
        oCon.Operation = SAPbouiCOM.BoConditionOperation.co_EQUAL
        oCon.CondVal = Trim(oForm.DataSources.DBDataSources.Item("@SST_OGAT").GetValue("U_VCode", 0))
        ...
        If objCFLEvent.BeforeAction = False Then
            Try
                If strCFL = "CFL_2" Then
                    oForm.DataSources.DBDataSources.Item("@SST_OGAT").SetValue("U_VCode", 0, oDT.GetValue("CardCode", 0))
                    oForm.DataSources.DBDataSources.Item("@SST_OGAT").SetValue("U_VDesc", 0, oDT.GetValue("CardName", 0))
                    AddPOCon()

                ElseIf strCFL = "CFL_4" Then
                    Try
                        oForm.DataSources.DBDataSources.Item("@SST_OGAT").SetValue("U_Series", 0, oDT.GetValue("Series", 0))
                        oForm.DataSources.DBDataSources.Item("@SST_OGAT").SetValue("U_PONum", 0, oDT.GetValue("DocNum", 0))
                        'oForm.DataSources.DBDataSources.Item("@SST_OGAT").SetValue("U_PODate", 0, objAddOn.objGenFunc.GetDateTimeValue(oDT.GetValue("DocDate", 0)).ToString("MM/dd/yy"))
                        oForm.DataSources.DBDataSources.Item("@SST_OGAT").SetValue("U_Docentry", 0, oDT.GetValue("DocEntry", 0))
                        AddItemCon()
                ...
                ElseIf strCFL = "CFL_3" Then
                    ...
                    strSQL = "select dscription,quantity,unitmsr,linenum,WhsCode,TAXCODE,PriceBefDi from POR1 where docentry = (select docentry from opor where docnum =" & oForm.DataSources.DBDataSources.Item("@SST_OGAT").GetValue("U_PONum", 0) & " and Doctype = 'I' and  series =" & oForm.DataSources.DBDataSources.Item("@SST_OGAT").GetValue("U_Series", 0) & " ) and itemcode = '" & oDT.GetValue("ItemCode", 0) & "'"
                    objRS.DoQuery(strSQL)
                    oForm.DataSources.DBDataSources.Item("@SST_GAT1").SetValue("U_ItmCode", 0, oDT.GetValue("ItemCode", 0))
                    oForm.DataSources.DBDataSources.Item("@SST_GAT1").SetValue("U_ItmDesc", 0, objRS.Fields.Item(0).Value)

                    oForm.DataSources.DBDataSources.Item("@SST_GAT1").SetValue("U_price", 0, objRS.Fields.Item(6).Value)
                    oForm.DataSources.DBDataSources.Item("@SST_GAT1").SetValue("U_whs", 0, objRS.Fields.Item(4).Value)
                    oForm.DataSources.DBDataSources.Item("@SST_GAT1").SetValue("U_tcode", 0, objRS.Fields.Item(5).Value)
                   

                    rs = objAddOn.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                    strSQL = "select sum(a.U_Qty) Qty from [@SST_GAT1] a"
                    strSQL = strSQL + " inner join [@SST_OGAT] b on a.docentry = b.docentry"
                    strSQL = strSQL + " where b.U_VCode = '" & Trim(oForm.DataSources.DBDataSources.Item("@SST_OGAT").GetValue("U_VCode", 0)) & "'  "
                    strSQL = strSQL + " and a.U_ItmCode = '" & Trim(oForm.DataSources.DBDataSources.Item("@SST_GAT1").GetValue("U_ItmCode", 0)) & "' and b.U_PONum = " & Trim(oForm.DataSources.DBDataSources.Item("@SST_OGAT").GetValue("U_PONum", 0))
                    rs.DoQuery(strSQL)
                    If rs.RecordCount > 0 Then
                        oForm.DataSources.DBDataSources.Item("@SST_GAT1").SetValue("U_Qty", 0, CDbl(objRS.Fields.Item(1).Value) - CDbl(rs.Fields.Item(0).Value))
                    Else
                        oForm.DataSources.DBDataSources.Item("@SST_GAT1").SetValue("U_Qty", 0, objRS.Fields.Item(1).Value)
                    End If

                    oForm.DataSources.DBDataSources.Item("@SST_GAT1").SetValue("U_UOM", 0, objRS.Fields.Item(2).Value)
                    oForm.DataSources.DBDataSources.Item("@SST_GAT1").SetValue("U_BrefNo", 0, objRS.Fields.Item(3).Value)
    ...
    Private Sub AddItemCon()
        objRS = objAddOn.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        strSQL = "select * from por1 where docentry = (select docentry from opor where docnum = " & oForm.DataSources.DBDataSources.Item("@SST_OGAT").GetValue("U_PONum", 0) & " and series = " & oForm.DataSources.DBDataSources.Item("@SST_OGAT").GetValue("U_Series", 0) & ")"
        objRS.DoQuery(strSQL)
