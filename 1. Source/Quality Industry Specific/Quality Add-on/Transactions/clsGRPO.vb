...
Public Class clsGRPO
    ...
    Private Sub InitializeFormComponent(ByVal FormUID)
        Try
            ...
            oForm = objAddOn.SBO_Application.Forms.Item(FormUID)
            oForm.Freeze(True)
            objItem = oForm.Items.Add("BtnGRN", SAPbouiCOM.BoFormItemTypes.it_BUTTON)
            objItem.Width = 80
            objItem.Top = oForm.Items.Item("1").Top
            objItem.Visible = True
            objItem.Left = 150 '(oForm.Width - (oForm.Width / 2) - 50)
            objItem.Height = 20
            BtnTax = oForm.Items.Item("BtnGRN").Specific
            BtnTax.Caption = "From Gate Entry"
            oForm.Refresh()
            oForm.Freeze(False)
            ...
            objItem.Top = oForm.Items.Item("1").Top
            objItem.Visible = True
            objItem.Left = 250 '(oForm.Width - (oForm.Width / 2) - 50)
            objItem.Height = 20
            BtnTax1 = oForm.Items.Item("BtnGRN1").Specific
            BtnTax1.Caption = "Sub Con GE"
            oForm.Refresh()
            oForm.Freeze(False)
                    ...
                    Select Case pVal.EventType
                        ...
                        Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                            If pVal.ItemUID = "1" Then
                                ...
                                'comment by sankar on 29.09.2011 for acessing from query
                                'grpoquantity = oMatrix.Columns.Item("11").Cells.Item(oMatrix.RowCount - 1).Specific.string
                                'end 

                                'MsgBox(grpoquantity)
                                'ItemCode = ""
                                'For m = 1 To oMatrix.RowCount - 1

                                '    ItmCode = oMatrix.Columns.Item("1").Cells.Item(m).Specific.string

                                '    If ItmCode.Contains(ActItem) = False Then
                                '        ActItem = oMatrix.Columns.Item("1").Cells.Item(m).Specific.string

                                '        If ItemCode = "" Then
                                '            ItemCode = ActItem
                                '        Else
                                '            ItemCode = ItemCode + "," + ActItem
                                '        End If
                                '    End If
                                'Next
                            End If
                        ...
                        Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                            If pVal.ItemUID = "BtnGRN" Then
                                ...
                                If oForm.Items.Item("4").Specific.string <> "" Then
                                    objAddOn.CFL.LoadScreen(oForm.Items.Item("4").Specific.string)
                                Else
                                    objAddOn.SBO_Application.SetStatusBarMessage("Select Supplier Code ", SAPbouiCOM.BoMessageTime.bmt_Short)
                                End If
                            End If

                            If pVal.ItemUID = "BtnGRN1" Then

                                If oForm.Items.Item("4").Specific.string <> "" Then
                                    objAddOn.CFL1.LoadScreen(oForm.Items.Item("4").Specific.string)
                                Else
                                    objAddOn.SBO_Application.SetStatusBarMessage("Select Supplier Code ", SAPbouiCOM.BoMessageTime.bmt_Short)
                                End If
                            End If
                            ...
                            If pVal.ItemUID = "1" And oForm.Mode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Then
                                ...
                                strSQL = "select docentry from opdn where docnum = '" & GRNNo & "' and series = '" & docseries & "' "
                                RS.DoQuery(strSQL)
                                'objAddOn.oCompany.GetNewObjectCode(strSQL)
    ...
    Public Function ReturnForm() As SAPbouiCOM.Form
        oForm = objAddOn.SBO_Application.Forms.GetForm(clsGRPO.formtype, 1)
        Return oForm
    End Function
